...
/* ─────────────────────────────────────────────
   ...
   ───────────────────────────────────────────── */
const watches = [
  { id: 1, image: "/watches/hero-main-removebg-preview.png", name: "Oyster Perpetual", model: "Model 04", dial: "Day Date", material: "Yellow Gold", tag: "Green Oysters", price: "$8,450", imageClass: "w-[152px] sm:w-[310px] md:w-[340px] lg:w-[370px]" },
  { id: 2, image: "/watches/watch-yellow-gold.png", name: "Oyster Perpetual", model: "Model 08", dial: "Master Date", material: "Rose Gold", tag: "Brown Leather", price: "$7,250", imageClass: "w-[96px] sm:w-[175px] md:w-[202px] lg:w-[232px]" },
  { id: 3, image: "/watches/watch-stealth-diver.png", name: "Oyster Perpetual", model: "Model 12", dial: "Submariner", material: "White Gold", tag: "Silver Frost", price: "$9,200", imageClass: "w-[96px] sm:w-[175px] md:w-[202px] lg:w-[232px]" },
];
...
function FloatingLabel({ text, position, delay = 0, phase2 }) {
  const posClasses = {
    tl: "-top-[8%] -left-[4%] sm:top-[4%] sm:left-[2%] md:-top-[10%] md:-left-[6%] lg:-top-[20%] lg:-left-[12%]",
    tr: "-top-[8%] -right-[4%] sm:top-[4%] sm:right-[2%] md:-top-[10%] md:-right-[6%] lg:-top-[20%] lg:-right-[12%]",
    bl: "-bottom-[8%] -left-[4%] sm:bottom-[16%] sm:left-[2%] md:-bottom-[4%] md:-left-[8%] lg:-bottom-[16%] lg:-left-[14%]",
    br: "-bottom-[8%] -right-[4%] sm:bottom-[16%] sm:right-[2%] md:-bottom-[4%] md:-right-[8%] lg:-bottom-[16%] lg:-right-[14%]",
  };
  ...
  return (
    <div className={`absolute z-30 overflow-hidden ${posClasses[position]}`}>
      <motion.span
        ...
        className="inline-block px-2.5 py-1 text-[7.5px] sm:px-4 sm:py-2 sm:text-[10.5px] tracking-[0.12em] sm:tracking-[0.16em] uppercase font-semibold text-[#1a4d2e] rounded-full shadow-sm border border-[#c5a56e]/20"
...
function WatchCard({ watch, direction, delay = 0, onClick }) {
  return (
    <motion.div
      ...
      className="flex-shrink-0 rounded-lg sm:rounded-2xl overflow-hidden cursor-pointer group"
      onClick={onClick}
      style={{ background: "rgba(255, 255, 255, 0.7)", border: "1px solid rgba(197, 165, 110, 0.12)", backdropFilter: "blur(10px)" }}
    >
      <div className="flex items-center gap-2.5 sm:gap-4 p-2 sm:p-4 relative">
        ...
        <div className="relative w-[48px] h-[48px] sm:w-[95px] sm:h-[95px] flex-shrink-0 rounded-lg sm:rounded-xl overflow-hidden flex items-center justify-center p-1 sm:p-1.5"
        ...
        <div className="flex-1 min-w-0 relative z-10">
          <h3 className="text-[10.5px] sm:text-[13px] tracking-[0.05em] sm:tracking-[0.07em] font-bold text-[#1a4d2e] truncate" style={{ fontFamily: "var(--font-geist)" }}>{watch.tag} &middot; {watch.material}</h3>
          <p className="text-[8.5px] sm:text-[11px] text-[#1a4d2e]/50 mt-0 sm:mt-1 tracking-[0.02em] truncate" style={{ fontFamily: "var(--font-geist)" }}>{watch.name} — {watch.dial}</p>
          <div className="flex items-center justify-between mt-1 sm:mt-2">
            <p className="text-[11.5px] sm:text-[15px] font-bold text-[#c5a56e]" style={{ fontFamily: "var(--font-geist)" }}>{watch.price}</p>
            <motion.div whileHover={{ x: 4 }} className="text-[#c5a56e]/30 group-hover:text-[#c5a56e] transition-colors duration-400 flex-shrink-0">
              <ArrowRight className="h-3 w-3 sm:h-4 sm:w-4" strokeWidth={2} />
            </motion.div>
          </div>
        </div>
      </div>
...
export default function ProductShowcase() {
  ...
  return (
    <section ref={sectionRef} id="showcase"
      className="relative h-screen min-h-screen flex flex-col justify-center overflow-hidden pt-12 pb-2 sm:py-12 lg:py-0 z-10 snap-start snap-always"
    ...
    >
      ...
      <div className="w-full max-w-[1440px] mx-auto px-4 sm:px-10 lg:px-14 py-0 sm:py-10 lg:py-8 flex items-center justify-center">
        <div className="relative w-full flex flex-col lg:flex-row items-center justify-between gap-5 sm:gap-6 lg:gap-14">

          {/* -- LEFT: Pillow + Spec Texts -- */}
          <div className="relative w-full lg:w-[52%] flex items-center justify-center pt-2 pb-0 sm:pt-0 sm:pb-0 min-h-[175px] sm:min-h-[460px] lg:min-h-[480px]">
            <motion.div style={{ opacity: showcasePillowOpacity }} id="showcase-pillow-container" className="relative w-[76%] max-w-[275px] sm:w-full sm:max-w-[500px] aspect-[4/3] flex items-center justify-center">
              ...
              <motion.div id="showcase-pillow-img"
                ...
                animate={
                  isScrollingAway ? { opacity: 0, x: 0, y: 0, scale: 1, rotate: 0 }
                    : isInView ? (showcasePhase2 ? { opacity: 1, x: 0, y: 0, scale: 1.04, rotate: 0 } : { opacity: 1, x: 0, y: 0, scale: 1, rotate: 0 })
                      : { opacity: 0, x: -65, y: -35, scale: 0.82, rotate: -10 }
                }
                transition={{ duration: 1, delay: 0.18, ease: [0.22, 1, 0.36, 1] }}
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 sm:top-auto sm:bottom-0 sm:translate-y-0 w-[62%] sm:w-[78%] z-[5]"
              >
                  ...
                  {carouselActive && (
                    ...
                    >
                      <Image src={activeWatch.image} alt={activeWatch?.name ? `${activeWatch.name} — ${activeWatch.dial}` : "Oyster Perpetual"}
                        ...
                        className={`${activeWatch.imageClass || "w-[100px] sm:w-[175px] md:w-[202px] lg:w-[232px]"} h-auto object-contain drop-shadow-[0_25px_45px_rgba(0,0,0,0.22)]`} />
          ...
          <motion.div initial={{ opacity: 0, x: 65, y: 35, scale: 0.82, rotate: 10 }}
            ...
            className="w-full lg:w-[45%] flex flex-col justify-center pt-1.5 sm:pt-0 lg:py-4"
          >
            <div className="relative mb-1.5 sm:mb-7">
              <div className="flex items-center justify-between">
                <span className="text-[9px] sm:text-[11px] tracking-[0.22em] sm:tracking-[0.32em] uppercase text-[#1a4d2e]/40 font-semibold block" style={{ fontFamily: "var(--font-geist)" }}>Our Collection</span>
                <div className="flex items-center gap-2 sm:gap-3">
                  <motion.button disabled={isFirst} whileHover={!isFirst ? { scale: 1.1, y: -1 } : {}} whileTap={!isFirst ? { scale: 0.92 } : {}} onClick={handlePrev}
                    className={`w-7 h-7 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-all duration-400 cursor-pointer ${isFirst ? "bg-white/40 opacity-30 cursor-not-allowed text-gray-400 border border-white/30" : "bg-white text-[#1a4d2e] hover:bg-[#1a4d2e] hover:text-white shadow-md shadow-black/5 hover:shadow-lg hover:shadow-[#1a4d2e]/15 border border-transparent group"}`}
                    aria-label="Previous Model"
                  ><ChevronLeft className="h-3.5 w-3.5 sm:h-5 sm:w-5 stroke-[2] group-hover:-translate-x-0.5 transition-transform duration-300" /></motion.button>
                  <motion.button disabled={isLast} whileHover={!isLast ? { scale: 1.1, y: -1 } : {}} whileTap={!isLast ? { scale: 0.92 } : {}} onClick={handleNext}
                    className={`w-7 h-7 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-all duration-400 cursor-pointer ${isLast ? "bg-[#c5a56e] opacity-30 cursor-not-allowed text-white border border-white/30" : "bg-white text-[#1a4d2e] hover:bg-[#1a4d2e] hover:text-white shadow-md shadow-black/5 hover:shadow-lg hover:shadow-[#1a4d2e]/15 border border-transparent group"}`}
                    aria-label="Next Model"
                  ><ChevronRight className="h-3.5 w-3.5 sm:h-5 sm:w-5 stroke-[2] group-hover:translate-x-0.5 transition-transform duration-300" /></motion.button>
                </div>
              </div>
              <h2 className="mt-1 sm:mt-4 text-xl sm:text-4xl md:text-5xl lg:text-[3.5rem] font-normal leading-tight" style={{ fontFamily: "var(--font-playfair)" }}>
                <span className="text-[#1a1a1a]">Pure </span><span className="text-[#1a4d2e]">Brilliance</span>
              </h2>
              <p className="mt-1 sm:mt-3 text-[10px] sm:text-[13px] text-[#1a4d2e]/40 max-w-[320px] leading-relaxed line-clamp-2 sm:line-clamp-none" style={{ fontFamily: "var(--font-geist)" }}>
                Each timepiece in our collection embodies generations of Swiss craftsmanship and uncompromising attention to detail.
              </p>
            </div>

            <div className="space-y-2.5 sm:space-y-4 mb-2 sm:mb-7">
            ...
            <div className="flex items-center justify-between text-[9px] sm:text-[10.5px] tracking-[0.18em] text-[#1a4d2e]/30 font-medium pt-0 sm:pt-2" style={{ fontFamily: "var(--font-geist)" }}>
